...
class Storage:
    ...
    def clear_user_room(self, user_id: int):
        """Отвязывает пользователя от комнаты (например, если тема/комната пропала)."""
        with closing(self._conn.cursor()) as cur:
            cur.execute("UPDATE users SET room_id=NULL WHERE user_id=?", (user_id,))
            self._conn.commit()

    def update_thread_id(self, room_id: int, new_thread_id: int):
        """Обновляет ID темы для комнаты — используется при восстановлении удалённой темы."""
        with closing(self._conn.cursor()) as cur:
    ...
    def clear_pending_invite(self, invite_id: int):
        """Удаляет приглашение напрямую по ID (например, если его комната больше не существует)."""
        with closing(self._conn.cursor()) as cur:
