...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if "Message thread not found" in str(e):
            logger.warning(f"Тема {thread_id} не найдена. Создаём новую для комнаты {row['room_id']}.")
            try:
                ...
                storage.update_thread_id(row["room_id"], new_thread_id)
                ...
                logger.error(f"Не удалось восстановить тему: {e2}")
    ...
    # Рассылаем всем участникам комнаты (кроме отправителя) с пометкой роли
...
def main():
    ...
    logger.info("🤖 Бот запущен (общий чат + восстановление тем без пустых сообщений)")
