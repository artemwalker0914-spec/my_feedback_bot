...
# -------------------- рассылка всем участникам комнаты (с жирной подписью) --------------------
...
# -------------------- безопасная отправка в тему (восстановление удалённой темы) --------------------
...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. Рассылаем всем участникам комнаты (кроме отправителя) с жирной подписью
    await broadcast_to_room(context, row["room_id"], user.id, message.text, role)


# -------------------- пересылка: ответ в теме -> всем участникам комнаты с подписью --------------------
...
def main():
    ...
    logger.info("🤖 Бот запущен (общий чат для комнаты + восстановление тем + жирные подписи)")
