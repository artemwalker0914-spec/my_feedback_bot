...
# -------------------- пересылка: пользователь -> тема + рассылка всем --------------------
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Проверяем тему и при необходимости восстанавливаем
    try:
        new_thread_id = await send_to_thread_safe(
            context,
            GROUP_CHAT_ID,
            thread_id,
            "",  # пустая заглушка для проверки существования темы
            user,
            row["room_id"],
        )
    ...
    # 2. Отправляем в группу учителей
    try:
        if message.text:
            # Текстовое – одним сообщением с подписью
            ...
            # Медиа – подпись и копия отдельно
            await context.bot.send_message(
                chat_id=GROUP_CHAT_ID,
                message_thread_id=thread_id,
                text=f"📩 {role_label(role)}:",
    ...
    # 3. Рассылаем всем участникам комнаты (кроме отправителя) с пометкой роли
    await broadcast_to_room(context, row["room_id"], user.id, message.text or "📎 Медиафайл", role)
...
async def handle_group_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in members:
        ...
        try:
            if message.text:
                # Текстовый ответ – одним сообщением с подписью
                await context.bot.send_message(
                    chat_id=member["user_id"],
...
def main():
    ...
    logger.info("🤖 Бот запущен (общий чат + восстановление тем)")
